<!-- 商品详情页面：展示商品信息、评价，支持加入购物车和立即购买 -->
<template>
  <div class="front-container" style="width: 50%">
    <!-- 商品基本信息区：图片 + 价格 + 描述 + 操作按钮 -->
    <div class="card" style="padding: 20px; display: flex; grid-gap: 20px; margin-bottom: 10px">
      ...
      <div style="flex: 1">
        ...
        <!-- 价格信息：原价（有折扣时划线）、折扣价、销量、库存 -->
        <div style="margin-bottom: 20px">
          <span v-if="data.goods.discountPrice" style="text-decoration: line-through; color: #999; font-size: 18px; margin-right: 10px">￥{{ data.goods.price }}</span>
          <span style="color: red; font-size: 18px">￥</span><b style="color: red; font-size: 30px">{{ data.goods.discountPrice || data.goods.price }}</b>
          <el-tag v-if="data.goods.discountPrice" type="danger" size="small" style="margin-left: 10px">限时折扣 {{ data.goods.discountEnd }}</el-tag>
    ...
    <div class="card" style="padding: 20px; margin-bottom: 50px">
      ...
      <div v-if="data.current === '商品评价'" style="min-height: 700px">
        ...
        <div v-if="data.commentList.length > 0" style="padding: 20px;">
          <!-- 循环渲染评论列表，每条评论包含头像、用户名、时间、评分、内容 -->
...
const data = reactive({
  ...
  id: router.currentRoute.value.query.id,   // 路由参数中的商品ID
  goods: {},          // 商品详细信息
  num: 1,             // 购买数量，默认1
  current: '商品详情',  // 当前激活的Tab：'商品详情' 或 '商品评价'
  commentList: [],    // 评论列表
  pageNum: 1,         // 评论分页-当前页码
  pageSize: 5,        // 评论分页-每页条数
  total: 0,           // 评论分页-总记录数
  userCollect: {},    // 当前用户对该商品的收藏记录（为空表示未收藏）
  form: {},           // 下单信息表单（deliverType, address）
