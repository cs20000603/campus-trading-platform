<!-- 商品列表页面：支持分类筛选、关键词搜索、分页浏览 -->
<template>
  <div class="front-container">
    <!-- 顶部筛选栏：分类标签 + 搜索框 -->
    <div style="display: flex; grid-gap: 20px; margin-bottom: 20px">
      <div style="flex: 1; display: flex; align-items: center; grid-gap: 20px">
        <!-- 分类标签列表：点击可按分类筛选 -->
        <div :class="{'active': null === data.categoryId }" @click="loadByCategory(null)"
             style="font-size: 16px; cursor: pointer; padding-bottom: 5px;">全部</div>
        <div :class="{'active': item.id === data.categoryId }" @click="loadByCategory(item.id)"
             style="font-size: 16px; cursor: pointer; padding-bottom: 5px;"
             v-for="item in data.categoryList" :key="item.id">{{ item.name }}</div>
      </div>
      <!-- 搜索框：支持自动补全 -->
      <div style="display: flex; align-items: center">
        <el-autocomplete
          v-model="data.name"
          :fetch-suggestions="querySearch"
          :trigger-on-focus="false"
          clearable
          @clear="load"
          @select="handleSelect"
          @keyup.enter="load"
          placeholder="请输入商品名称搜索"
          style="width: 300px"
        />
        <el-button @click="load" type="primary" style="height: 40px; margin-left: 5px">搜 索</el-button>
      </div>
    </div>
    <!-- 商品展示区：网格布局显示商品卡片 -->
    <div v-if="data.total > 0">
      <el-row :gutter="20">
        <el-col :span="6" v-for="item in data.tableData" :key="item.id">
          <!-- 商品卡片：点击跳转详情页 -->
          <div @click="router.push('/front/goodsDetail?id=' + item.id)" class="card goods-item"
               style="width: 100%; padding: 0; border-radius: 5px; margin-bottom: 20px">
            <img :src="item.img" alt="" style="width: 100%; height: 260px; border-radius: 5px 5px 0 0">
            <div style="padding: 5px">
              <div class="line1" style="font-size: 18px; margin-bottom: 5px">{{ item.name }}</div>
              <div>
                <!-- 原价（有折扣时划线显示） -->
                <span v-if="item.discountPrice" style="text-decoration: line-through; color: #999; font-size: 14px">￥{{ item.price }}</span>
                <!-- 当前价格（优先显示折扣价） -->
                <span style="color: red">￥</span><b style="color: red; font-size: 20px">{{ item.discountPrice || item.price }}</b>
                <!-- 折扣标签 -->
                <el-tag v-if="item.discountPrice" type="danger" size="small" style="margin-left: 5px">限时折扣</el-tag>
                <span style="margin-left: 10px; color: #666">销量：{{ item.saleCount }}</span>
              </div>
            </div>
          </div>
        </el-col>
      </el-row>

      <!-- 分页组件 -->
      <div v-if="data.total > 0">
        <el-pagination style="background-color: white; width: fit-content; padding: 5px 10px; border-radius: 5px" @current-change="load" layout="total, prev, pager, next" v-model:page-size="data.pageSize" v-model:current-page="data.pageNum" :total="data.total"/>
      </div>
    </div>
    <!-- 空数据提示 -->
    <div v-else style="padding: 50px; text-align: center; color: #666">暂无数据...</div>
  </div>
</template>

<script setup>
import { reactive } from "vue";
import router from "@/router";
import request from "@/utils/request";

// 响应式数据对象
const data = reactive({
  name: router.currentRoute.value.query.name,           // 搜索关键词（可从路由参数获取）
  shopId: router.currentRoute.value.query.shopId,       // 店铺ID（从路由参数获取，用于筛选特定店铺的商品）
  shopType: router.currentRoute.value.query.shopType,   // 店铺类型（从路由参数获取）
  tableData: [],      // 商品列表数据
  pageNum: 1,         // 当前页码
  pageSize: 8,        // 每页显示条数
  total: 0,           // 总记录数
  categoryId: null,   // 当前选中的分类ID（null表示全部）
  categoryList: []    // 分类列表
})

// 按分类筛选商品
const loadByCategory = (categoryId) => {
  data.categoryId = categoryId
  load()
}

// 搜索自动补全：根据输入的关键词从后端获取建议列表
const querySearch = (queryString, cb) => {
  if (!queryString) {
    cb([])
    return
  }
  request.get('/goods/suggest', { params: { keyword: queryString } }).then(res => {
    const list = (res.data || []).map(name => ({ value: name }))
    cb(list)
  }).catch(() => cb([]))
}

// 选中自动补全的建议项时触发搜索
const handleSelect = (item) => {
  data.name = item.value
  load()
}

// 分页查询商品数据（仅查询上架状态的商品）
const load = () => {
  request.get('/goods/selectPage', {
    params: {
      pageNum: data.pageNum,
      pageSize: data.pageSize,
      status: '上架',
      name: data.name,
      categoryId: data.categoryId,
      shopId: data.shopId || null
    }
  }).then(res => {
    data.tableData = res.data?.list
    data.total = res.data?.total
  })
}
load()

// 加载分类列表：根据店铺类型加载对应的分类
const loadCategory = () => {
  if (data.shopType) {
    // 有店铺类型时，按类型加载分类
    request.get('/category/selectByShopType', { params: { shopType: data.shopType } }).then(res => {
      data.categoryList = res.data || []
    })
  } else {
    // 无店铺类型时，加载全部分类
    request.get('/category/selectAll').then(res => {
      data.categoryList = res.data
    })
  }
}
loadCategory()

// 清除URL中的查询参数（保持URL干净）
const clearPathParam = () => {
  let url = location.href
  url = url.replace(/(\?|#)[^'"]*/, '');           //去除参数
  window.history.pushState({},0, url);
}
clearPathParam()
</script>

<style scoped>
.active {
  font-weight: bold;
  color: #0c9c7a;
  border-bottom: 2px solid #0c9c7a;
}
.goods-item {
  cursor: pointer;
  transition: all 0.5s;
}
.goods-item:hover {
  transform: scale(1.05);
}
</style>