<template>
  <div>
    <!-- 商品管理页面：按名称/分类筛选、新增、编辑、删除商品，支持AI生成简介 -->
      ...
      <el-table tooltip-effect="dark myEff" :data="data.tableData" stripe>
        ...
        <el-table-column prop="price" label="价格">
          <template #default="scope">
            ...
            <b v-if="scope.row.discountPrice" style="color: red; margin-left: 5px">折扣:{{ scope.row.discountPrice }}元</b>
          </template>
        </el-table-column>
    ...
    <el-dialog title="商品信息" width="50%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="formRef" :model="data.form" :rules="data.rules" label-width="80px" style="padding-right: 30px; padding-top: 20px">
        ...
        <el-form-item label="折扣价">
          <el-input-number :min="0" v-model="data.form.discountPrice" placeholder="不填表示无折扣"></el-input-number>
        </el-form-item>
        ...
        <el-form-item prop="description" label="简介">
          <div style="display: flex; gap: 8px; width: 100%">
            <el-input type="textarea" :rows="3" v-model="data.form.description" placeholder="请输入简介" style="flex: 1"></el-input>
...
const data = reactive({
  ...
  viewVisible: false, // 详情查看弹窗
  content: null,      // 商品详情HTML内容
  aiLoading: false,   // AI生成加载状态
  rules: {
    name: [
